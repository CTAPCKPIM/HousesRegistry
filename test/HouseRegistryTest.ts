...
describe('House Registry:', () => {
  ...
  it('Should be creating a new house', async ()=> {
    const funct = await houseRegistry.connect(accountTwo).listHouse(123, 123, 123, accountTwo.address, '123');
    const data = await funct.wait();
  ...
  it('Should be returned the error: "Price, square not can bee 0"', async ()=> {
    ...
    await expect(houseRegistry.listHouse(0, 0, 0, accountTwo.address, '123')).to.be.revertedWith(message);
  });

  it('Should be returned the error: "This ID already exists"', async ()=> {
    const message = 'This ID already exists';
    await expect(houseRegistry.connect(accountThree).
      listHouse(122, 122, 122, accountTwo.address, '122')).to.be.revertedWith(message);
  });

  it('Should be delisted by owner', async ()=> {
    const message = 'You no the owner';
    await expect(houseRegistry.connect(accountTwo).delistHouse(houseId)).to.be.revertedWith(message);
  });
